...
| `visual-book` | ビジュアルブック | 視覚的な2段組のドキュメント（B5，横組） | TypeScript/Markdown |
...
2 回目以降の実行では，キャッシュされたリポジトリが fast-forward 可能な場合に自動更新されます．

> [!WARNING]
> カスタムテンプレートは，現在のユーザ権限で任意のコードを実行できます．
> ローカルディレクトリまたは git リポジトリのいずれかを指定する場合，信頼できるテンプレートのみを使用するようにしてください．
